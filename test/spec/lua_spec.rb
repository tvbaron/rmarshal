...
describe 'lua' do
  ...
  describe 'produce 1 object to a JSON file' do
    ...
    it 'produces (with --lua)' do
      AppHelper.exec_prog ["--lua", "#{context.datadir}/script01.lua", "--json", "--pretty", "#{AppHelper.tmpdir}/out.json"]
      expect(File.read("#{AppHelper.tmpdir}/out.json")).to eq(File.read("#{context.datadir}/expect01.json"))
    end

    it 'produces (without --lua)' do
      AppHelper.exec_prog ["#{context.datadir}/script01.lua", "--json", "--pretty", "#{AppHelper.tmpdir}/out.json"]
      expect(File.read("#{AppHelper.tmpdir}/out.json")).to eq(File.read("#{context.datadir}/expect01.json"))
    end
  end
  ...
  describe 'produce 2 objects to 2 JSON files' do
    ...
    it 'produces (with --lua)' do
      AppHelper.exec_prog ["--lua", "#{context.datadir}/script02.lua", "--json", "--pretty", "#{AppHelper.tmpdir}/out1.json", "--json", "--pretty", "#{AppHelper.tmpdir}/out2.json"]
    ...
    it 'produces (without --lua)' do
        AppHelper.exec_prog ["#{context.datadir}/script02.lua", "--json", "--pretty", "#{AppHelper.tmpdir}/out1.json", "--json", "--pretty", "#{AppHelper.tmpdir}/out2.json"]
        expect(File.read("#{AppHelper.tmpdir}/out1.json")).to eq(File.read("#{context.datadir}/expect01.json"))
        expect(File.read("#{AppHelper.tmpdir}/out2.json")).to eq(File.read("#{context.datadir}/expect02.json"))
    end
  end
